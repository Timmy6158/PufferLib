...
def load_config(parser, config_path="config.yaml"):
    ...
    for key in valid_keys:
        ...
        config[key] = {**fn_subconfig, **default[key], **pkg_subconfig, **env_subconfig}
    ...
    pufferlib.utils.validate_args(
        make_env.func if isinstance(make_env, functools.partial) else make_env,
        config["env"],
    )
    ...
    parser.add_argument(
        "--use_rnn", default=False, action="store_true", help="Wrap policy with an RNN"
    )
...
def train(args, env_module, make_env):
    ...
    if args.backend == "clean_pufferl":
        ...
        model = RecurrentPPO(
            ...
            n_epochs=args.train.update_epochs,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Configuration in config.yaml to use",
    )
    parser.add_argument(
        "--backend",
        type=str,
        default="clean_pufferl",
        help="Train backend (clean_pufferl, sb3)",
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    parser.add_argument(
        "--no-render", action="store_true", help="Disable render during evaluate"
    )
    parser.add_argument(
        "--vec",
        "--vector",
    ...
    parser.add_argument(
        "--wandb-entity", type=str, default="poke-re", help="WandB entity"
    )
    parser.add_argument(
        "--wandb-project", type=str, default="pufferlib", help="WandB project"
    )
